...
CREATE TABLE `event_db`.`block_sync` (
  ...
  `last_sync_timestamp` timestamp NOT NULL COMMENT 'last synced block timestamp',
  `last_finalized_number` bigint unsigned NOT NULL COMMENT 'last finalized block number',
  `last_finalized_timestamp` timestamp NOT NULL COMMENT 'last finalized block timestamp',
